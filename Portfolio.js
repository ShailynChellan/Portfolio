const projects = {
  project1: {
    images: ['Assets/ERS1.JPG', 'Assets/ERS2.JPG', 'Assets/ERS3.JPG', 'Assets/ERS4.JPG', 'Assets/ERS5.JPG', 'Assets/ERS6.JPG', 'Assets/ERS7.JPG', 'Assets/ERS8.JPG', 'Assets/ERSA1.JPG', 'Assets/ERSA2.JPG', 'Assets/ERSA3.JPG'],
  ...
  project2: {
   ...
   details: `<h3>Music Archive</h3>
				...
				<h4><u>Project Details:</u></h4>
				<p>Music Archive is a web application I developed to give users a space to share their honest thoughts, reviews, and opinions on the music they love. The idea behind it is simple: a personal music library where you can reflect on albums, discover new sounds, and connect with others who share your taste. The platform currently supports a single-user experience as it's still in development, but the long-term vision is much broader.</p>
				<p>My goal is to evolve Music Archive into a fully interactive, multi-user platform where people around the world can create accounts, write reviews, recommend albums, and showcase their music preferences. I plan to implement a milestone system to track and reward engagement such as badges or counters for reviewing a certain number of albums.</p>
				<p>One of the key upcoming features is collaborative playlist building. Users will be able to create and share playlists directly on the site. If someone enjoys a shared playlist, they’ll be able to instantly export it to their preferred music streaming service like Spotify. This will involve integrating with APIs like Spotify's Web API for seamless syncing.</p>
				<p>Eventually, I aim to scale Music Archive into a broader Media Archive, expanding the platform to support not only music lovers but also movie fans and book readers—offering a central space where people can express their tastes across multiple forms of media. This is a long-term passion project that reflects both my technical skills and my love for creative expression.</p>
				<h5><u>How The Site Works: </u></h5>
				<p>Using Music Archive is simple and intuitive. To start, you just enter the album’s name along with the artist, genre, release year, and your tags; these tags can be anything you choose, whether they reflect the album’s mood, themes, or help with more specific genre-based sorting. You’ll also rate the album out of 5 stars and have the option to mark it as a favorite if it holds a special place for you.</p>
				<p>Once you add an album, the app fetches real-time metadata using the iTunes Search API. This includes the official album cover, a full tracklist, and 30-second audio previews for each song, which are playable directly on the site. Each album is displayed as a card on your personal archive, creating a visually appealing, scrollable collection. All added albums are stored in your browser’s local storage, meaning your archive remains intact even after closing and reopening the site.</p>
				<p>You also have full control over your entries. Albums can be edited at any time, whether you spot a typo or want to change your rating after a few more listens. You can even go through each tracklist and highlight your favorite songs by clicking the heart icon next to them.</p>
				<p>While this version is focused on single-user functionality, there’s a lot more planned. Future features include multi-user support, public and private playlist creation, AI-powered album recommendations tailored to your tastes, activity-based milestones that recognize your engagement with the platform, and more.</p>
		        <h5><u>How The Site Was Made: </u></h5>
				<p>Music Archive was built entirely in React.js using JavaScript, HTML, and CSS, with a focus on creating a smooth, responsive, and intuitive user experience. It is my first of many future projects and one that I would continue to expand on as my knowledge and skillset with React grows.</p>
				<p>For storing album data, I implemented localStorage to ensure albums persist across sessions without requiring backend infrastructure, which is perfect for a personal single-user prototype. Album covers, tracklists, and 30-second previews are pulled automatically via the iTunes Search API, which I integrated using asynchronous requests with Axios. This allows users to input just the album title and artist, and instantly receive accurate, real-time metadata.</p>
				<p>Each album is represented as a reusable card component, which visually displays key info like rating, genre, tags, and favorite status. Clicking a card opens a modal window with expanded details, including a full tracklist, favorite song toggles (using heart icons), and embedded audio previews. I also added edit functionality to update or fix album entries without re-adding them, and ensured real-time UI updates through React state management.</p>
				<p>On the UI side, I used custom CSS to build a clean, dark/light theme that enhances readability and aesthetics while maintaining responsiveness across different screen sizes. Accessibility and user clarity were important goals, so everything from tag input to preview playback was built with usability in mind.</p>
				<p>This project has helped me sharpen my skills in React, API integration, client-side storage, and UI/UX design, and it’s far from finished. Future development plans include full authentication for multi-user access, cloud-based storage, playlist sharing, and AI-powered music suggestions.</p>`
  }
};
